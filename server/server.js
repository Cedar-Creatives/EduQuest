...
if (missingVars) {
  ...
  console.error(
    "OPENROUTER_API_KEY: Required for AI features (quiz generation, AI teachers)"
  );
  console.error("Note: Firebase authentication is handled on the client-side");
  process.exit(1);
}
...
if (firebase.clientSideOnly) {
  console.log("=== FIREBASE: Client-side authentication only ===");
  console.log("=== FIREBASE: Server running without Admin SDK ===");
} else {
  console.log("=== FIREBASE: Admin SDK mode ===");
}
...
app.get("/api/health", (req, res) => {
  ...
  const health = {
    ...
    firebase: {
      projectId: process.env.FIREBASE_PROJECT_ID || "client-side-only",
      mode: firebase.clientSideOnly ? "client-side-auth" : "admin-sdk",
    },
    environment: {
...
app.get("/api/help", (req, res) => {
  ...
  res.json({
    ...
    gettingStarted: {
      ...
      step3:
        'Use the token in Authorization header: "Authorization: Bearer YOUR_TOKEN"',
    },
    availableEndpoints: {
      authentication: {
    ...
    quickStart: {
      command1: "curl -X POST http://localhost:5000/api/auth/create-test-user",
      command2: "Copy the accessToken from response",
      command3:
        'For Windows CMD: curl -X POST http://localhost:5000/api/quiz/subjects -H "Content-Type: application/json" -H "Authorization: Bearer YOUR_TOKEN" -d "{"name": "Mathematics", "description": "Basic mathematics", "avgTime": 20}"',
      command4:
        'For PowerShell: curl -X POST http://localhost:5000/api/quiz/subjects -H "Content-Type: application/json" -H "Authorization: Bearer YOUR_TOKEN" -d \'{"name": "Mathematics", "description": "Basic mathematics", "avgTime": 20}\'',
    },
    troubleshooting: {
      issue1:
        "If you get 401/403 errors, make sure you have a valid accessToken",
      issue2: "Get a token by calling /api/auth/create-test-user first",
      issue3: "Make sure to use http://localhost:5000 (not 5173) for API calls",
      issue4:
        "On Windows, use proper JSON escaping or try PowerShell instead of CMD",
    },
  });
});
