...
class OpenRouterService {
  ...
  async generateQuizQuestions(subject, difficulty, count = 5) {
    ...
    try {
      ...
      if (error.response) {
        ...
        throw new Error(`Failed to generate quiz questions: ${error.response.data.error?.message || error.response.statusText || 'API error'}`);
      } else if (error.request) {
        // The request was made but no response was received
        console.error('API Request Error: No response received');
        throw new Error('Failed to generate quiz questions: No response from AI service');
      } else {
  ...
  async explainAnswer(question, userAnswer, correctAnswer, explanation) {
    ...
    try {
      ...
      if (error.response) {
        // The request was made and the server responded with a status code outside of 2xx range
        console.error('API Response Error:', error.response.data);
        throw new Error(`Failed to generate answer explanation: ${error.response.data.error?.message || error.response.statusText || 'API error'}`);
      } else if (error.request) {
        // The request was made but no response was received
        console.error('API Request Error: No response received');
        throw new Error('Failed to generate answer explanation: No response from AI service');
      } else {
        // Something happened in setting up the request
        throw new Error(`Failed to generate answer explanation: ${error.message}`);
      }
    }
  }
  ...
  async generateSubjectDescription(subjectName) {
    ...
    try {
      ...
      if (error.response) {
        // The request was made and the server responded with a status code outside of 2xx range
        console.error('API Response Error:', error.response.data);
        throw new Error(`Failed to generate subject description: ${error.response.data.error?.message || error.response.statusText || 'API error'}`);
      } else if (error.request) {
        // The request was made but no response was received
        console.error('API Request Error: No response received');
        throw new Error('Failed to generate subject description: No response from AI service');
      } else {
        // Something happened in setting up the request
        throw new Error(`Failed to generate subject description: ${error.message}`);
      }
    }
  }
  ...
  async summarizeNote(noteContent) {
    ...
    try {
      ...
      if (error.response) {
        // The request was made and the server responded with a status code outside of 2xx range
        console.error('API Response Error:', error.response.data);
        throw new Error(`Failed to summarize note: ${error.response.data.error?.message || error.response.statusText || 'API error'}`);
      } else if (error.request) {
        // The request was made but no response was received
        console.error('API Request Error: No response received');
        throw new Error('Failed to summarize note: No response from AI service');
      } else {
        // Something happened in setting up the request
  ...
  async searchNotes(query, notes) {
    ...
    try {
      ...
      And these notes:
      ${notes.map((note, index) => `${index + 1}. ${note.title}: ${note.content.substring(0, 200)}...`).join('\n')}
      ...
      if (error.response) {
        // The request was made and the server responded with a status code outside of 2xx range
        console.error('API Response Error:', error.response.data);
        throw new Error(`Failed to search notes: ${error.response.data.error?.message || error.response.statusText || 'API error'}`);
      } else if (error.request) {
        // The request was made but no response was received
        console.error('API Request Error: No response received');
